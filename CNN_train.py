    ...
    def __getitem__(self, index):
        try:
            ...
            if self.has_id_column:
                ...
                img_name = 0 # do not need to load image name in this case
            ...
            return image, attr_vector, label, img_name
...
print(f"Size (number of elements): {attributes.size}")

# Display the shape (dimensions of the array)
print(f"Shape (dimensions): {attributes.shape}")
...
train_dataset = OurDataset(
    csv_file="image_metadata.csv",  # CSV file with filenames and labels
    root_dir="manipulated_images",        # Directory with the images
    attributes = attributes,
    has_id_column = False,
    transform=train_transforms                                # Transformations
)
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for data, att, targets, id in train_loader:
            data, att, targets,id = data.to(device), att.to(device), targets.to(device), id.to(device)
...
def validate_model(model, val_loader, criterion):
    ...
    with torch.no_grad():
        for data, att, targets, id in val_loader:
...
if __name__ == "__main__":
    ...
    torch.save(model.state_dict(), "trained_model.pth.tar")
    model.eval()
